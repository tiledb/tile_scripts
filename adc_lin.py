...
def analyze_pulse(samples, threshold_fraction=0.5):
    """
    Analyze a pulse waveform.

    Returns:
        peak_value      : maximum sample value
        peak_index      : index of maximum
        center_of_mass  : weighted center
        fwhm            : full width at half maximum (in sample units)
    """
    ...
    peak_index = samples.index(peak_value)

    # --- Center of mass (robust pulse center) ---
    total = sum(samples)
    if total > 0:
        center_of_mass = sum(i * v for i, v in enumerate(samples)) / total
    else:
        center_of_mass = None
    ...
    above_half = [i for i, v in enumerate(samples) if v >= half_max]
    ...
    return peak_value, peak_index, center_of_mass, fwhm
...
print("=" * 60)
print(f"{'MD':<6}{'Ch':<6}{'HG_Peak':<10}{'HG_PeakIdx':<10}{'HG_Center':<12}{'HG_FWHM':<8}{'LG_Peak':<10}{'LG_PeakIdx':<10}{'LG_Center':<12}{'LG_FWHM':<8}")
print("-" * 60)
...
for md in range(firstMD, firstMD + nMD):
    for adc in range(nchanperMD):
        ...
        hg_peak, hg_peak_idx, hg_center, hg_width = analyze_pulse(all_hg_data[idx])
        lg_peak, lg_peak_idx, lg_center, lg_width = analyze_pulse(all_lg_data[idx])

        print(f"{md:<6}{adc:<6}{hg_peak:<10}{hg_peak_idx:<10}{hg_center:<12.2f}{hg_width:<8}{lg_peak:<10}{lg_peak_idx:<10}{lg_center:<12.2f}{lg_width:<8}")
