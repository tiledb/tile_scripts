...
class PPr:
    ...
    def read_eye(self, verbose=False):
        ...
        ver = 2
        hor = 2
        psc = 3          # Match original implementation
        ut  = 0

        # Original implementation only used 12 links
        max_link = 16
        ...
        while rdy != 0xFFFF:
            ...
            if time.time() - start_time > timeout:
                raise TimeoutError("Timeout waiting for eye scan READY")

            time.sleep(0.1)

        # ---------------------------
        # Flush pipeline / prepare
        # ---------------------------
        if verbose:
        ...
        if verbose:
            print(f"Matrix size: lanes={max_link}, v={v}, h={h}")

        # Keep EXACT return structure compatibility:
        # eye[lane][vertical][horizontal]
        eye = [[[0.0 for _ in range(h)] for _ in range(v)] for _ in range(max_link)]
        ...
        for rr in range(max_link):
            ...
            for vv in range(v):
                ...
                for hh in range(0, h, n):
                    ...
                    # Original firmware duplicated value across n bins
                    for ii in range(n):
